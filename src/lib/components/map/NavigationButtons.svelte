<script lang="ts">
	...
	const otherProviders = $derived(navigationProviders.filter((p) => p.id !== defaultProviderId));
</script>
...
<div class="flex flex-col gap-1">
	<div class="relative flex gap-1.5">
		<a
			href={getProviderUrl(defaultProvider, lat, lng)}
			target="_blank"
		...
		>
			<MapPin size={15} />
			<span>Ouvrir dans {defaultProvider.shortLabel}</span>
		</a>
		<button
			onclick={(e) => {
				e.stopPropagation();
